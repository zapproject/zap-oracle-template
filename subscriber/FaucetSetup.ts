...
const FAUCET_ADDR:string = "0x25eaf1ff3107bf9c98c0c6de0185bca637ed9265";
...
async function main() {
	...
	await token.allocate({to: FAUCET_ADDR, amount: 10000, from: owner}).then((txid: any) => { 
 		console.log('Allocation to Faucet, Hash:', txid.transactionHash); 
 	});
	await token.allocate({to: owner, amount: 10000, from: owner}).then((txid: any) => { 
 		console.log('Allocation to Owner, Hash:', txid.transactionHash); 
 	});

	// approve bondage contract for the delegate bond
	await token.approve({to: BONDAGE_ADDR, amount: 10000, from: owner}).then((txid: any) => { 
 		console.log('Approval to Faucet, Hash:', txid.transactionHash); 
 	});

	// delegated bond to the contract
	await bondage.delegateBond({provider: ORACLE_ADDR, endpoint: "zapprice", dots: 10, subscriber: FAUCET_ADDR, from: owner}).then((txid: any) => { 
